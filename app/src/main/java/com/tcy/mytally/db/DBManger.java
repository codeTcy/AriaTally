...
public class DBManger {
    ...
    public static float getMaxMoneyOneDayInMonth(int year, int month, int kind) {
        ...
        Cursor cursor = db.rawQuery(sql, new String[]{year + "", month + "", kind + ""});
        if (cursor.moveToFirst()) {
            float v = cursor.getFloat(cursor.getColumnIndex("sum(money)"));
            result = v;
        }
    ...
    public static List<BarChartItemBean> getSumMoneyOneDayInMonth(int year, int month, int kind) {
        ...
        String sql = "select day,sum(money) from accounttb where year=? and month=? and kind=? group by day order by day asc ";
        Cursor cursor = db.rawQuery(sql, new String[]{year + "", month + "", kind + ""});
        while (cursor.moveToNext()) {
            int day = cursor.getInt(cursor.getColumnIndex("day"));
            float sum = cursor.getFloat(cursor.getColumnIndex("sum(money)"));
